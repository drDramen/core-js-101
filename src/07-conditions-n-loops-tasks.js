...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    const char = str[i];
    if (i === str.indexOf(char) && i === str.lastIndexOf(char)) {
      return char;
    }
  }
...
function isCreditCardNumber(ccn) {
  let sum = 0;
  const stringCCN = String(ccn).split('').reverse();

  for (let i = 0; i < stringCCN.length; i += 1) {
    const number = Number(stringCCN[i]);
    if (i % 2 === 0) {
      sum += number;
    } else if (number > 4) {
      sum += number * 2 - 9;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const char = str[i];
    if (stack.length === 0 && !openBrackets.includes(char)) {
      return false;
    }
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1.length; i += 1) {
    output.push(new Array(m1.length).fill(0));
